...
/**
 * Clase con métodos para validar diferentes tipos de datos (NIF, fecha, edad, código postal, email, nombre)
 */
public class UserDataValidations {
    ...
    public static boolean isAlphabetic(String str) {
        ...
        return isAlpha;  // Devuelve true si todos los caracteres son letras
    }
    ...
    public static boolean isNumeric(String str) {
        ...
        return isNum;  // Devuelve true si todos los caracteres son dígitos
    }
    ...
    public static int calculateAge(String birthDate) {
        ...
        String anioCumpleConString = "" + birthDate.charAt(5) + birthDate.charAt(6) + birthDate.charAt(7) + birthDate.charAt(8);
    ...
    public static boolean checkPostalCode(String zip) {
        if (zip.length() != 5 || !isNumeric(zip)) {  // El código postal debe ser numérico y tener 5 caracteres
            return false;
        }
        return true;  // Si cumple con la condición, devuelve true
    }

    // Verifica si el formato del correo electrónico es válido
    public static boolean checkEmail(String email) {
        boolean isString = false;
        boolean comprobarEsNum = UserDataValidations.isAlphabetic(email);  // Verifica si es alfabético
        if (comprobarEsNum) {
            if (email.contains("@") && (email.contains(".com") || email.contains(".es"))) {  // Verifica que contenga "@" y un dominio válido
                isString = true;
            }
        }
        return isString;  // Devuelve true si el correo tiene un formato válido
    }

    // Verifica si el nombre tiene solo caracteres alfabéticos y una longitud menor a 30
    public static boolean checkName(String name) {
        boolean esCorrecta = false;
        boolean comprobarEsNum = UserDataValidations.isAlphabetic(name);  // Verifica si el nombre es alfabético
        int longitud = 30;
        if (comprobarEsNum && name.length() < longitud) {  // Verifica que el nombre sea corto (menos de 30 caracteres)
            esCorrecta = true;
        }
        return esCorrecta;  // Devuelve true si el nombre es válido
    }
}